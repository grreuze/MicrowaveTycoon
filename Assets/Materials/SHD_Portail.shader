...
/////// Diffuse:
                ...
                float4 node_7271 = _Time + _TimeEditor;
                float2 node_8320 = (i.uv0+node_7271.g*float2(-0.3,-0.2));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_8320, _Noise));
                float3 node_1732 = lerp(float3((i.uv0+node_7271.g*float2(0.7,0)),0.0),_Noise_var.rgb.rgb,_node_8867);
...
/////// Diffuse:
                float NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 node_9698 = _Time + _TimeEditor;
                float2 node_8320 = (i.uv0+node_9698.g*float2(-0.3,-0.2));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_8320, _Noise));
                float3 node_1732 = lerp(float3((i.uv0+node_9698.g*float2(0.7,0)),0.0),_Noise_var.rgb.rgb,_node_8867);
                float4 _Smoke_var = tex2D(_Smoke,TRANSFORM_TEX(node_1732, _Smoke));
                float3 diffuseColor = _Smoke_var.rgb;
